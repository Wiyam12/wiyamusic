...
class JustAudioMediaKit extends JustAudioPlatform {
  ...
  /// Last applied mpv `af` filter chain (re-applied when new players start).
  static String _audioFilter = '';
  ...
  /// Sets the mpv audio-filter chain on all active players (used for equalizer).
  static Future<void> setAudioFilter(String af) async {
    _audioFilter = af;
    final instance = JustAudioPlatform.instance;
    if (instance is! JustAudioMediaKit) return;
    await Future.wait(
      instance._players.values.map((player) => player.setAudioFilter(af)),
    );
  }
  ...
  Future<AudioPlayerPlatform> init(InitRequest request) async {
    ...
    await player.ready();
    if (_audioFilter.isNotEmpty) {
      await player.setAudioFilter(_audioFilter);
    }
